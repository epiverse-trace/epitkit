...
- Es importante cargar previamente una librería quecontenga el pipe. Por ejemplo: `magrittr`, `dplyr`, `tidyr` o `purrr`. 
...
#dataframe de ejemplo
 ...
 #resumen por columnas
    por_dia <- df %>%
      ...
      summarize(f_l = mean(f_latidos), f_r = mean(f_respiratoria))
```
...
por_dia %>% ungroup() #si bien desagrupa el objeto para imprimirlo, mientras no se almacene  el objeto serguirá agrupado
por_dia %>% mutate(id = row_number())
```
...
```{r}
por_dia_sin_grupo <- por_dia %>% ungroup() #ahora estamos almacenando el desagrupamiento

por_dia_sin_grupo %>% mutate(id = row_number())
```
Como puede observar ahora si tenemos cada fila con su propio id.

En caso que no quiera almacenar el cambio de desagrupar en el objeto principal, puede aplicarlo cada vez que lo necesite 
 
```{r}
por_dia %>% ungroup() %>% mutate(id = row_number())
```
 
Como puede ver cada fila tiene su propio id.
